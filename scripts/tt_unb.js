...
function processRecords(records, levels, categories, players) {
    const validLevelIDs = new Set([
        "29vk433d", "xd434j29", "xd4px00d", "xd07mgxw", 
        "rw6n84rd", "29vg2m19", "n93gv7nd", "z988y6g9", 
        "rdnp60q9", "xd0pxk0w", "ldypnykd", "n93kolew", 
        "gdrqme69", "nwl7ql09", "rw623qnd"
    ]);
...
function createListButton() {
    ...
    button.addEventListener('click', () => {
        window.location.href = 'TimeTrial.html';
    });
...
Promise.all([
    ...
    // Filter only unbeaten records
    const unbeatenRecords = processedRecords.filter(record => record.beatenBy === 'Unbeaten');
    
    populateTable(unbeatenRecords);
